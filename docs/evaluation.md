...
当前机器生成结果位于 `evaluation/recruitmatch-v1-results.json` 和 `evaluation/recruitmatch-ai-v1-results.json`。AI 基准会真实调用可控 Fake Model、LLM 解析器、语义检索、混合排序与受约束解释链路；Fake Model 只用于离线确定性和无外部费用，不是线上模型效果证明。语义分由候选人与不同岗位的证据重合实际计算，150 个样本中 140 个产生区分度、85 个的 Top 3 顺序发生变化。当前 `hybrid-v1` 在该合成集上为 Top-1 97.33%、Top-3 100%、引用有效率 100%。复现命令：
